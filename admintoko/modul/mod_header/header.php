...
switch($_GET[act]){
  // Tampil header
  default:
    $banner = mysql_query("SELECT value1 FROM setting WHERE tipe='slider'");
    $r    = mysql_fetch_array($banner);
    
    echo "<h2>Edit Gambar Slider</h2>
         <form method=POST action='$aksi?module=header&act=status'>
    ";
    if ($r[value1]=="Y"){
      echo "<b>Aktifkan Slider?</b> : <input type=radio name='aktif' value='Y' checked>Y  
                                      <input type=radio name='aktif' value='N'> N &nbsp;&nbsp;&nbsp;<input type=submit class=tombol value=Simpan>";
    }
    else{
    ...
    while ($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td>$no</td>
                ...
                <td><a href=?module=header&act=editheader&id=$r[id_header]>Edit</a> | 
	                  <a href='$aksi?module=header&act=hapus&id=$r[id_header]' onclick='return confirm(\"Hapus gambar header ini?\");'>Hapus</a>
		        </tr>";
    $no++;
    }
    echo "</table>";
